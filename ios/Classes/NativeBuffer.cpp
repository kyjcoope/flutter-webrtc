...
NativeBuffer::NativeBuffer(int capacity, int initial_max_buffer_size)
    : capacity_(static_cast<size_t>(capacity)),
      current_max_frame_buffer_size_(
          static_cast<size_t>(initial_max_buffer_size)),
      write_index_(0),
      read_index_(0),
      count_(0) {
  if (capacity <= 0 || initial_max_buffer_size <= 0) {
    throw std::invalid_argument(
        "Capacity and initial_max_buffer_size must be positive.");
  }

  frames_.reserve(capacity_);
  for (size_t i = 0; i < capacity_; ++i) {
    frames_.emplace_back(
        std::make_unique<MediaFrame>(current_max_frame_buffer_size_));
  }
}

int NativeBuffer::pushInternal(const uint8_t* data,
                               size_t data_size,
                               MediaType type,
...
int NativeBuffer::pushVideoFrame(const uint8_t* data,
                                 size_t data_size,
                                 int width,
                                 int height,
                                 uint64_t frame_time,
  ...
  return pushInternal(data, data_size, MEDIA_TYPE_VIDEO, metadata_union,
                      frame_time);
}

int NativeBuffer::pushAudioFrame(const uint8_t* data,
                                 size_t data_size,
                                 int sample_rate,
                                 int channels,
                                 uint64_t frame_time) {
  ...
  return pushInternal(data, data_size, MEDIA_TYPE_AUDIO, metadata_union,
                      frame_time);
}
