...
// A plain C struct view that matches Dart's MediaFrameNative layout
// Keep this in sync with lib/bindings/native_bindings.dart
...
// Use thread_local storage to avoid repeated allocations; contents are overwritten on each pop
...
FFI_PLUGIN_EXPORT uintptr_t popNativeBufferFFI(const char* key) {
    ...
    // Map NativeBuffer's internal frame to a plain struct expected by Dart
    ...
    if (frame->mediaType == MEDIA_TYPE_VIDEO) {
        g_plainFrame.width = frame->metadata.video.width;
        g_plainFrame.height = frame->metadata.video.height;
        g_plainFrame.rotation = frame->metadata.video.rotation;
        // Use frameType as the color format for raw pixel frames
        g_plainFrame.colorFormat = frame->metadata.video.frameType;
    } else {
        g_plainFrame.width = 0;
        g_plainFrame.height = 0;
        g_plainFrame.rotation = 0;
        g_plainFrame.colorFormat = 0;
    }
