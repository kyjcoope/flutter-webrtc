...
static std::unordered_map<std::string, std::unique_ptr<NativeBuffer>>
    g_nativeBuffers;
...
FFI_PLUGIN_EXPORT int initNativeBufferFFI(const char* key,
                                          int capacity,
                                          int maxBufferSize) {
  ...
  if (g_nativeBuffers.find(skey) == g_nativeBuffers.end()) {
    try {
      g_nativeBuffers[skey] =
          std::make_unique<NativeBuffer>(capacity, maxBufferSize);
...
FFI_PLUGIN_EXPORT int pushVideoNativeBufferFFI(const char* key,
  ...
  int result = buffer_ptr->pushVideoFrame(buffer, dataSize, width, height,
                                          frameTime, rotation);
  return handlePushResult(result, skey);
}

FFI_PLUGIN_EXPORT int pushAudioNativeBufferFFI(const char* key,
                                               const uint8_t* buffer,
                                               size_t dataSize,
                                               int sampleRate,
                                               int channels,
  ...
  int result = buffer_ptr->pushAudioFrame(buffer, dataSize, sampleRate,
                                          channels, frameTime);
  return handlePushResult(result, skey);
}
...
FFI_PLUGIN_EXPORT bool registerDartPort(const char* channel_name,
                                        int64_t port) {
