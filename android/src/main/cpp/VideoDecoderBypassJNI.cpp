...
Java_com_cloudwebrtc_webrtc_record_RawFrameCapturer_initNativeBuffer(JNIEnv *env, jclass clazz, jstring jTrackId, jint capacity, jint bufferSize) {
...
Java_com_cloudwebrtc_webrtc_record_RawFrameCapturer_pushFrame(JNIEnv *env, jclass clazz, jstring jTrackId, jobject buffer,
...
Java_com_cloudwebrtc_webrtc_record_RawFrameCapturer_freeNativeBuffer(JNIEnv *env, jclass clazz, jstring jTrackId) {
    const char* key = env->GetStringUTFChars(jTrackId, NULL);
    if (key) {
