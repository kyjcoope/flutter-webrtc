...
Java_com_cloudwebrtc_webrtc_record_RawFrameCapturer_initNativeBuffer(
    JNIEnv* env,
    jclass clazz,
    jstring jTrackId,
    jint capacity,
...
Java_com_cloudwebrtc_webrtc_record_RawFrameCapturer_freeNativeBuffer(
    JNIEnv* env,
    jclass clazz,
    jstring jTrackId) {
  const char* key = env->GetStringUTFChars(jTrackId, NULL);
...
// ---- RawAudioCapturer JNI bindings ----

JNIEXPORT jint JNICALL
Java_com_cloudwebrtc_webrtc_record_RawAudioCapturer_initNativeBuffer(
    JNIEnv* env,
    jclass clazz,
    jstring jTrackId,
    jint capacity,
...
JNIEXPORT jlong JNICALL
Java_com_cloudwebrtc_webrtc_record_RawAudioCapturer_pushAudioFrame(
    JNIEnv* env,
    jclass clazz,
    jstring jTrackId,
    jobject buffer,
  ...
  int ffi_result =
      pushAudioNativeBufferFFI(key, buf, dataSize, sampleRate, channels,
                               static_cast<uint64_t>(frameTimeMs));
  env->ReleaseStringUTFChars(jTrackId, key);
  return static_cast<jlong>(ffi_result);
}

JNIEXPORT void JNICALL
Java_com_cloudwebrtc_webrtc_record_RawAudioCapturer_freeNativeBuffer(
    JNIEnv* env,
    jclass clazz,
    jstring jTrackId) {
  const char* key = env->GetStringUTFChars(jTrackId, NULL);
