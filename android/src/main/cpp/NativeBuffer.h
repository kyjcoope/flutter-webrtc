...
typedef enum { MEDIA_TYPE_VIDEO = 0, MEDIA_TYPE_AUDIO = 1 } MediaType;
...
class NativeBuffer {
 public:
  ...
  int pushVideoFrame(const uint8_t* data,
                     size_t data_size,
                     int width,
                     int height,
                     uint64_t frame_time,
                     int rotation);
  int pushAudioFrame(const uint8_t* data,
                     size_t data_size,
                     int sample_rate,
                     int channels,
                     uint64_t frame_time);
 ...
 private:
  int pushInternal(const uint8_t* data,
                   size_t data_size,
                   MediaType type,
